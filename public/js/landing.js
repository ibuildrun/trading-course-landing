const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    function tick() {
      ...
      scrollyItems.forEach(s => { const r = s.el.getBoundingClientRect(); const denom = s.el.offsetHeight - vh;
      ...
      revealWords.forEach(p => { const trigger = vh * 0.78;
    ...
    (function () {
      ...
      let W,H,ratio,candles=[];
      function resize(){ ratio=window.devicePixelRatio||1; W=canvas.clientWidth=window.innerWidth; H=canvas.clientHeight=window.innerHeight; canvas.width=W*ratio; canvas.height=H*ratio; ctx.setTransform(ratio,0,0,ratio,0,0); }
      ...
      function init(){ resize(); const n=Math.max(10,Math.floor(W/90)); candles=Array.from({length:n},(_,i)=>seed(i,n)); }
      function draw(){ ctx.clearRect(0,0,W,H); candles.forEach(c=>{ c.y-=c.speed; if(c.y+c.body+c.wick<-20) c.y=H+40;
        ...
        ctx.fillStyle=`rgba(${col},${c.alpha})`; ctx.fillRect(c.x-c.w/2,c.y,c.w,c.body); }); requestAnimationFrame(draw); }
