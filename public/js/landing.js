const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    /* ---------- Smooth scroll: Lenis (локальный) + rAF-фолбэк, без зависимостей от React ---------- */
    (function initLenis(){
      if (reduceMotion || window.__lenis) return;
      const s = document.createElement('script');
      s.src = '/trading-course-landing/js/lenis.min.js';
      s.onload = function(){
        try {
          const l = new window.Lenis({ duration: 1.15, easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)), smoothWheel: true });
    ...
    function smoothTo(top){
      top = Math.max(0, top);
      if (window.__lenis) { window.__lenis.scrollTo(top, { duration: 1.2 }); return; }
      if (scrollAnim) cancelAnimationFrame(scrollAnim);
      const start = window.scrollY, dist = top - start;
      if (Math.abs(dist) < 2) return;
      const dur = clamp(500 + Math.abs(dist) * 0.12, 600, 1300);
    ...
    function scrollToId(id){ const t = document.querySelector(id); if(!t) return;
      smoothTo(t.getBoundingClientRect().top + window.scrollY - 80); }

    // Делегированный перехват ВСЕХ якорных кликов (capture — выигрывает у любых других обработчиков)
    document.addEventListener('click', (e) => {
      ...
      if (id === '#top') { e.preventDefault(); e.stopImmediatePropagation(); smoothTo(0); closeMenu(); return; }
      const t = document.querySelector(id); if (!t) return;
      e.preventDefault(); e.stopImmediatePropagation();
      scrollToId(id); closeMenu();
    }, true);
